...
"$BT/zipalign" -f -p 4 "$OUT/app-unsigned.apk" "$OUT/SpinMe-v0.1.0-aligned-unsigned.apk"
...
if [[ -n "${SPINME_KEYSTORE:-}" ]]; then
  ...
  "$BT/apksigner" sign \
    ...
    --out "$OUT/SpinMe-v0.1.0.apk" \
    "$OUT/SpinMe-v0.1.0-aligned-unsigned.apk"
  "$BT/apksigner" verify --verbose --print-certs "$OUT/SpinMe-v0.1.0.apk"
  sha256sum "$OUT/SpinMe-v0.1.0.apk"
else
  echo "Unsigned aligned APK: $OUT/SpinMe-v0.1.0-aligned-unsigned.apk"
fi
